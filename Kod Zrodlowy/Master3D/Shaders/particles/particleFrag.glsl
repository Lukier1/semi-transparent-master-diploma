...
{
	float deltaZ = 2; // Gruboœæ obiektu przezroczystego
	vec3 w0 = -normalize(X);
	n = normalize(n);
...
void main() {
	//TODO Dodac wysylanie rozdzielczoœci

	discardWhenIsCovered();
	
	vec4 fragColor = texture2D(diffuse_texture, texCoord);
	vec3 Lr;
	Lr  = vec3(fragColor)*ambientLight;

	vec3 t = transmitive*(vec3(1)-Lr);
	float computeAlpha = alpha*fragColor.w;
	//LIGHT PART
	
	float netCoverage = computeAlpha*(1.0 - dot(t, vec3(1.0/3.0)));
	
	weight = clamp(pow(10*(1-0.99*gl_FragCoord.z)*netCoverage, 3), 0.01, 150.0);
	
	//Color of object
	alphaAccRGBA = vec4(computeAlpha*Lr, netCoverage)*weight; 
	
	//Transimssion
	//alphaBckD.xyz = vec3(1)-vec3(computeAlpha)+computeAlpha*vec3(t);// computeAlpha*(vec3(1)-t)*(1.0/3.0);
	alphaBckD.xyz = computeAlpha*(vec3(1)-t);
	alphaBckD.w = 0.0f;
	
	//Difraction
	if(eta != 1.0) {
		vec2 refractPix = refractOffset(normalCamera, positionCamera, eta);
		alphaDelta = refractPix* netCoverage*1/63;
	}
	else {
		alphaDelta = vec2(0,0);
	}
